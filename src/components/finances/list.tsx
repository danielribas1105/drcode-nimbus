		...
		<div className="flex flex-col gap-3">
			{transations.length > 0 ? (
				transations.map((t, i) => (
					...
					>
						<span className={`flex justify-center items-center w-8 h-8 p-1.5 rounded-full ${t.tipo === 'receita' ? 'bg-green-500' : 'bg-red-500'}`}>{t.tipo === 'receita' ? <IconTrendingUp/> : <IconTrendingDown/>}</span>
